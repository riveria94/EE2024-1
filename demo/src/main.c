...
/*	uint32_t debounceTime = 0;
	...
	{
		/* <---- Display update ----> */
		// oled_putChar(0, 0, 0x21, OLED_COLOR_WHITE, OLED_COLOR_BLACK);
...
/*		light = light_read();
		...
		if(getMsTicks() - startTime > 1000){
			light = light_read();
			sprintf(sens_value, "light : %u", light);
			oled_putString(0, 0, sens_value, OLED_COLOR_WHITE, OLED_COLOR_BLACK);
